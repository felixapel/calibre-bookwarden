...
class ValkeyQueue(JobQueue):
    ...
    def _cleanup_memory_jobs(self) -> None:
        ...
        if len(self._fallback_db) > 100:
            finished = [(jid, j) for jid, j in self._fallback_db.items() if j.get("status") in ("completed", "failed")]
    ...
    async def enqueue(self, task: str, payload: dict[str, Any]) -> str:
        ...
        if self._client:
            try:
                set_result: Any = await self._client.set(f"job:{job_id}", json.dumps(job_data))
                rpush_result: Any = await self._client.rpush("queue:jobs", job_id)
